...
string getGreedyMove(){  //editing
		int k=0 ;int maxy=-1 ;
		p2p pos_pair ;
        if(killerTownHallMoves.size() != 0){pos_pair=killerTownHallMoves[0];}
        else if(killerPlayerMoves.size() != 0){
			for(int i=0;i<killerPlayerMoves.size();i++) 
				if(killerPlayerMoves[i].second.first>maxy){
					maxy=killerPlayerMoves[i].second.first ;
					k=i ;
				}
			pos_pair = killerPlayerMoves[k];
		}
        else if(cannonAttacks.size() != 0){
			for(int i=0;i<cannonAttacks.size();i++) 
				if(cannonAttacks[i].second.first>maxy){
					maxy=cannonAttacks[i].second.first ;
					k=i ;
				}
			pos_pair = make_pair(get<1>(cannonAttacks[k].first),cannonAttacks[k].second);
		}
        else if(cannonCaptures.size() != 0){pos_pair = cannonCaptures[rand()%cannonCaptures.size()];}
        else{
			maxy=SHORTMAX ; //here it is miny
			for(int i=0;i<validMoves.size();i++) 
				if(validMoves[i].second.first<maxy){
					maxy=validMoves[i].second.first ;
					k=i ;
				}			
			pos_pair = validMoves[k];
		}
